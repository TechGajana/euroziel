  ...
  return (
    ...
    >
      <div className="flex w-max animate-ticker items-center gap-x-8 whitespace-nowrap hover:[animation-play-state:paused]">
        {items.map((item, i) => (
          <div key={i} className="flex shrink-0 items-center gap-2 text-[13px] text-[#A8C8F0]">
            <Diamond
  ...
  return (
    <div className="group rounded-xl border border-[rgba(74,144,217,0.14)] bg-[#0D1B2A] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[rgba(74,144,217,0.35)] hover:shadow-[0_8px_40px_rgba(74,144,217,0.12)] sm:p-7">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="font-heading text-[13.5px] font-bold text-[#E8EDF5]">{name}</div>
          <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11.5px] text-[#A8C8F0]">
            <span>{info}</span>
            {country ? <CountryFlag code={country} className="h-[10px] w-4 rounded-[2px]" /> : null}
          </div>
        </div>
      </div>
    </div>
...
function FeatureCard({
  ...
  return (
    ...
    >
      ...
      <h3 className="mb-2.5 text-[15px] font-bold leading-snug text-[#E8EDF5]">{title}</h3>
...
export default function HomePage() {
    ...
    <>
      ...
      `,
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse, rgba(74,144,217,0.10) 0%, transparent 68%)',
          }}
        />
          ...
          <div className="grid w-full grid-cols-1 items-center gap-6 md:grid-cols-2 md:gap-8 lg:gap-[60px]">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className="animate-fade-up-d2 mb-6 max-w-[490px] text-[14.5px] font-light leading-[1.85] text-[#A8C8F0] sm:text-[16px] md:text-[15.5px] lg:text-[17px]">
                EuroZiel connects ambitious Indian students directly to Germany&apos;s top
                public universities — with guidance from students who are already there.
              </p>
              ...
              <div className="animate-fade-up-d4 mb-6 flex items-center gap-3">
                ...
                >
                  {/* WhatsApp SVG */}
                  <svg className="h-4 w-4 shrink-0 transition-colors duration-200 group-hover:text-[#25D366]" viewBox="0 0 24 24" fill="currentColor">
  ...
  `}</style>
      ...
      <section
        ref={whyRef.ref}
        className="px-4 py-16 sm:px-[5%] sm:py-24"
      >
        <div className="mx-auto max-w-[1240px]">
          <EyeBrow className={whyRef.inView ? 'animate-fade-up' : 'opacity-0'}>
            Why EuroZiel
          </EyeBrow>
          <SectionTitle className={whyRef.inView ? 'animate-fade-up-d1' : 'opacity-0'}>
            Everything you need,
            <br />
          ...
          <div className="relative mt-10 grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-[rgba(74,144,217,0.16)] bg-[rgba(74,144,217,0.12)] min-[480px]:grid-cols-2 md:mt-[60px] lg:grid-cols-3">
            {[
              ['01', 'Germany-Exclusive Focus', "Specialised in Germany's top public universities — deep domain expertise, not a generalist agency."],
              ['02', 'Peer Mentors in Germany', 'Direct guidance from students enrolled at TU Munich, Berlin, and other top German institutions.'],
      ...
      >
        <div className="mx-auto max-w-[1240px]">
          <EyeBrow className={processRef.inView ? 'animate-fade-up' : 'opacity-0'}>
            How It Works
          </EyeBrow>
          <SectionTitle className={processRef.inView ? 'animate-fade-up-d1' : 'opacity-0'}>
            Your journey in{' '}
            ...
            {PROCESS_STEPS.map(({ n, title, sub, celebrate }, i) => (
              <div
                ...
                style={{
                  ...
                  transform: processRef.inView ? 'translateY(0)' : 'translateY(20px)',
              ...
              >
                ...
                <div className="mb-0.5 flex flex-wrap items-center justify-center gap-1 font-heading text-[12px] font-bold leading-snug text-[#E8EDF5] sm:text-[13px]">
                  {celebrate && (
                    <PartyPopper className="h-3.5 w-3.5 shrink-0 text-[#4A90D9]" strokeWidth={2} />
                  )}
                  {title}
                </div>
                <div className="text-[11px] leading-snug text-[#A8C8F0] sm:text-[12px]">{sub}</div>
              </div>
            ))}
          </div>
        </div>
